...
def aggregate_by_timeframe(df: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    df_resampled[['cum_positive', 'cum_neutral', 'cum_negative', 'cum_reviews', 'cum_ananlyzed', 'cum_weighted_sentiment', 'cum_sentiment_score', 'cum_rating']] = \
        df_resampled[['Positive', 'Neutral', 'Negative', 'n_reviews', 'n_analyzed', 'weighted_sentiment', 'sentiment_score', 'rating']].cumsum()
        
    df_resampled['avg_rating'] = df_resampled['cum_rating'] / df_resampled['cum_reviews'].replace(0, pd.NA)
    
    for label in ['positive', 'neutral', 'negative']:
        df_resampled[f'{label}_ratio'] = df_resampled[f'cum_{label}'] / df_resampled['cum_ananlyzed'].replace(0, pd.NA)
    df_resampled['sentiment_index'] = (df['cum_weighted_sentiment'] / df['cum_sentiment_score'].replace(0, pd.NA) + 1) * 50
    
    logger.info(f"Aggregation complete. Resulting data has {len(df_resampled)} records.")
